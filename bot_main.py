...
async def admin_manual_checkins_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Начинает диалог обработки ручных заявок. Отображает список ожидающих заявок.
    ...
    pending_requests = db.get_pending_manual_checkin_requests() # Эта функция должна возвращать список sqlite3.Row
    ...
    for req_idx, req in enumerate(pending_requests): # req это sqlite3.Row
        # req - это sqlite3.Row, доступ к полям по имени или индексу
        ...
        user_id_val = req['user_id'] # Предполагаем, что user_id всегда есть
        request_id_val = req['request_id'] # Предполагаем, что request_id всегда есть
...
async def admin_handle_final_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "admin_confirm_final_yes":
        ...
        await context.bot.send_message(
            chat_id=admin_id, 
            text="Обработка завершена\\.\nЧтобы просмотреть другие заявки, введите /admin_manual_checkins\\.", # Экранированы точки
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return ConversationHandler.END
...
def main() -> None:
    ...
    application.add_handler(manual_checkin_conv_handler) # Добавлен обработчик ручной отметки пользователя
    application.add_handler(admin_manual_checkins_conv_handler) # Добавлен обработчик админки ручных заявок
    ...
    application.add_handler(CommandHandler("restart", restart_command)) # Убедитесь, что restart_command определена
