...
def is_admin(user_id: int) -> bool:
    """
    (ИСПРАВЛЕННАЯ ВЕРСИЯ)
    Корректно проверяет, является ли пользователь администратором,
    преобразуя строку из конфига в список чисел.
    """
    try:
        # 1. Читаем переменную из конфига как строку
        admin_ids_str = str(config.ADMIN_TELEGRAM_IDS)

        # 2. Разделяем строку по запятым и превращаем каждую часть в число
        admin_ids_list = [int(id_str.strip()) for id_str in admin_ids_str.split(',') if id_str.strip()]
        
        # 3. Сравниваем ЧИСЛО со СПИСКОМ ЧИСЕЛ.
        return user_id in admin_ids_list

    except (ValueError, TypeError):
        # Если в конфиге ошибка (не числа), считаем, что админов нет
        return False

class AdminFilter(BaseFilter):
    def filter(self, message: Update) -> bool:
        """
        Фильтр, который делегирует проверку единой функции is_admin.
        """
        # Он больше не думает сам. Он спрашивает у is_admin.
...
# Состояния для диалога редактирования сессий
EDIT_AWAIT_NAME, EDIT_SELECT_USER, EDIT_SELECT_PERIOD, EDIT_SELECT_SESSION, EDIT_AWAIT_NEW_TIME, EDIT_CONFIRM = range(10, 16)
...
async def edit_checkout_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Начинает диалог редактирования сессии."""
    await update.message.reply_text(
        "📝 Введите ФИО или часть ФИО сотрудника, чью сессию вы хотите отредактировать."
    )
    return EDIT_AWAIT_NAME

async def edit_receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Принимает ФИО, ищет в базе и предлагает выбрать."""
    user_input_name = update.message.text
    found_users = db.find_users_by_name(user_input_name)
    
    if not found_users:
        await update.message.reply_text(
            f"❌ Сотрудник с именем, похожим на '{user_input_name}', не найден. Попробуйте еще раз или введите /cancel для отмены."
        )
        return EDIT_AWAIT_NAME

    if len(found_users) == 1:
        user = found_users[0]
        context.user_data['edit_target_user_id'] = user['telegram_id']
        context.user_data['edit_target_user_name'] = user['application_full_name']
        
        # ИЗМЕНЕНИЕ: Вызываем следующую функцию в диалоге
        return await edit_show_period_selection(update, context)

    keyboard = []
    for user in found_users:
        callback_data = f"edit_select_user:{user['telegram_id']}"
        keyboard.append([InlineKeyboardButton(user['application_full_name'], callback_data=callback_data)])
    
    keyboard.append([InlineKeyboardButton("Отмена", callback_data="edit_cancel")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await update.message.reply_text(
        "Найдено несколько сотрудников. Пожалуйста, выберите нужного:",
        reply_markup=reply_markup
    )
    return EDIT_SELECT_USER


async def edit_show_period_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Показывает кнопки выбора периода для выбранного пользователя."""
    query = update.callback_query
    if query:
        await query.answer()

    user_name = context.user_data.get('edit_target_user_name', 'Неизвестный')

    keyboard = [
        [InlineKeyboardButton("🗓️ За последнюю неделю", callback_data="edit_period:week")],
        [InlineKeyboardButton("🗓️ За последний месяц", callback_data="edit_period:month")],
        [InlineKeyboardButton("🔢 Показать последние 5", callback_data="edit_period:last5")],
        # ИЗМЕНЕНИЕ: Кнопка теперь ведет на шаг назад, а не отменяет все
        [InlineKeyboardButton("↩️ Назад к выбору сотрудника", callback_data="edit_back_to_name")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)

    message_text = f"✅ Выбран сотрудник: <b>{user_name}</b>\n\nЗа какой период вы хотите посмотреть его завершенные сессии?"
    ...
    return EDIT_SELECT_PERIOD

async def edit_select_user_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Обрабатывает выбор пользователя из списка нескольких найденных."""
    query = update.callback_query
    await query.answer()

    user_id = int(query.data.split(':')[1])
    
    # Мы не можем просто взять имя с кнопки, т.к. оно может быть неполным
    # Поэтому делаем еще один запрос в БД, чтобы получить полное имя
    user_info = db.get_user(user_id)
    if not user_info:
        await query.edit_message_text("❌ Произошла ошибка. Пользователь не найден. Диалог завершен.")
        return ConversationHandler.END

    context.user_data['edit_target_user_id'] = user_id
    context.user_data['edit_target_user_name'] = user_info['application_full_name']

    return await edit_show_period_selection(update, context)

SESSIONS_PER_PAGE = 5

async def edit_build_session_list(context: ContextTypes.DEFAULT_TYPE, offset: int) -> tuple[str, InlineKeyboardMarkup]:
    """Собирает текст и клавиатуру для страницы списка сессий."""
    
    # Получаем данные из контекста диалога
    user_name = context.user_data.get('edit_target_user_name', 'Неизвестный')
    all_sessions = context.user_data.get('edit_sessions_list', [])
    total_sessions = len(all_sessions)

    # Вычисляем, какие сессии показать на этой странице
    sessions_to_show = all_sessions[offset : offset + SESSIONS_PER_PAGE]
    
    # Инициализируем пустой список для кнопок
    keyboard = []
    
    # Формируем заголовок
    page_info = f"({offset + 1}-{offset + len(sessions_to_show)} из {total_sessions})"
    message_text = f"Сессии для <b>{user_name}</b> {page_info}:\n"

    # Формируем кнопки для каждой сессии
    for session in sessions_to_show:
        try:
            # Парсим время из строк
            checkin_dt = datetime.strptime(session['check_in_time'], '%Y-%m-%d %H:%M:%S')
            checkout_dt = datetime.strptime(session['check_out_time'], '%Y-%m-%d %H:%M:%S')

            # Умное форматирование даты
            if checkin_dt.date() == checkout_dt.date():
                # Сессия в пределах одного дня
                date_str = checkin_dt.strftime('%d.%m')
                time_str = f"{checkin_dt.strftime('%H:%M')} - {checkout_dt.strftime('%H:%M')}"
                button_text = f"🗓️ {date_str} | {time_str}"
            else:
                # Сессия переходит через полночь
                date_str_in = checkin_dt.strftime('%d.%m')
                date_str_out = checkout_dt.strftime('%d.%m')
                time_str = f"{checkin_dt.strftime('%H:%M')} - {checkout_dt.strftime('%H:%M')}"
                button_text = f"🗓️ {date_str_in} → {date_str_out} | {time_str}"

        except (ValueError, TypeError):
            button_text = f"Ошибка данных для сессии ID: {session['session_id']}"

        # callback_data содержит ID сессии, которую мы хотим редактировать
        callback_data = f"edit_select_session:{session['session_id']}"
    ...
    if offset > 0:
        ...
        prev_offset = max(0, offset - SESSIONS_PER_PAGE)
        nav_buttons.append(InlineKeyboardButton("⬅️ Назад", callback_data=f"edit_paginate:{prev_offset}"))
    
    if offset + SESSIONS_PER_PAGE < total_sessions:
        # Есть следующая страница
        next_offset = offset + SESSIONS_PER_PAGE
        nav_buttons.append(InlineKeyboardButton("➡️ Вперед", callback_data=f"edit_paginate:{next_offset}"))
    
    if nav_buttons:
        keyboard.append(nav_buttons)

    # Кнопка "Назад к выбору периода"
    keyboard.append([InlineKeyboardButton("↩️ Назад к выбору периода", callback_data="edit_back_to_period")])
    
    return message_text, InlineKeyboardMarkup(keyboard)
 

async def edit_back_to_name_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Возвращает пользователя к шагу ввода ФИО."""
    query = update.callback_query
    await query.answer()
    
    # Очищаем данные о выбранном пользователе, чтобы не было путаницы
    context.user_data.pop('edit_target_user_id', None)
    context.user_data.pop('edit_target_user_name', None)
...
async def edit_back_to_period_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Возвращает пользователя к шагу выбора периода."""
    # Мы не удаляем данные о пользователе, просто вызываем функцию,
    # которая отрисовывает этот шаг.
    return await edit_show_period_selection(update, context)


async def edit_select_period_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Обрабатывает выбор периода и показывает ПЕРВУЮ страницу списка сессий."""
    query = update.callback_query
    await query.answer()
    
    period = query.data.split(':')[1]
    user_id = context.user_data['edit_target_user_id']
    user_name = context.user_data['edit_target_user_name']

    # Получаем ВСЕ сессии за период
    sessions = db.get_completed_sessions_for_user(user_id, period)

    if not sessions:
        await query.edit_message_text(f"У сотрудника <b>{user_name}</b> не найдено завершенных сессий за выбранный период.", parse_mode=ParseMode.HTML)
        context.user_data.clear()
        return ConversationHandler.END

    # Сохраняем полный список сессий в контекст для пагинации
    context.user_data['edit_sessions_list'] = sessions
    
    # Вызываем "сборщик" для самой первой страницы (смещение 0)
    message_text, reply_markup = await edit_build_session_list(context, offset=0)
    ...
    # Переходим в состояние выбора конкретной сессии
    return EDIT_SELECT_SESSION

async def edit_paginate_sessions_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Вызываем наш "сборщик" с новым смещением
    message_text, reply_markup = await edit_build_session_list(context, offset)
    ...
    try:
        ...
        # Эта заглушка нужна, если пользователь слишком часто кликает
        logger.warning(f"Не удалось обновить сообщение при пагинации: {e}")

    return EDIT_SELECT_SESSION # Остаемся в том же состоянии

async def edit_session_selected_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Обрабатывает выбор конкретной сессии и запрашивает новое время."""
    query = update.callback_query
    await query.answer()

    session_id = int(query.data.split(':')[1])
    context.user_data['edit_target_session_id'] = session_id
    
    # Находим информацию о сессии из списка, который мы уже сохранили
    all_sessions = context.user_data.get('edit_sessions_list', [])
    selected_session = next((s for s in all_sessions if s['session_id'] == session_id), None)

    if not selected_session:
        await query.edit_message_text("❌ Ошибка: сессия не найдена. Диалог завершен.")
        context.user_data.clear()
        return ConversationHandler.END

    # Форматируем текущее время ухода для отображения
    current_checkout_dt = datetime.strptime(selected_session['check_out_time'], '%Y-%m-%d %H:%M:%S')
    current_checkout_str = current_checkout_dt.strftime('%d.%m.%Y %H:%M')

    message_text = (
        f"Выбрана сессия от <b>{current_checkout_dt.strftime('%d.%m.%Y')}</b>.\n"
        f"Текущее время ухода: <code>{current_checkout_str}</code>\n\n"
        "Пожалуйста, введите новое время ухода в формате <b>ДД.ММ.ГГГГ ЧЧ:ММ</b>\n"
        "Например: <code>27.07.2025 18:30</code>"
    )
    
    keyboard = [[InlineKeyboardButton("↩️ Назад к списку сессий", callback_data="edit_back_to_sessions")]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.edit_message_text(message_text, reply_markup=reply_markup, parse_mode=ParseMode.HTML)
    return EDIT_AWAIT_NEW_TIME

async def edit_receive_new_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Принимает новое время, валидирует и просит подтверждения."""
    new_time_str = update.message.text
    
    try:
        # Валидация формата
        new_checkout_dt = datetime.strptime(new_time_str, '%d.%m.%Y %H:%M')
        # Сохраняем валидный объект datetime
        context.user_data['edit_new_checkout_dt'] = new_checkout_dt
    except ValueError:
        await update.message.reply_text(
            "❌ Неверный формат. Пожалуйста, введите время в формате <b>ДД.ММ.ГГГГ ЧЧ:ММ</b> (например, <code>27.07.2025 18:30</code>).",
            parse_mode=ParseMode.HTML
        )
        return EDIT_AWAIT_NEW_TIME # Остаемся в том же состоянии

    user_name = context.user_data.get('edit_target_user_name', '')
    
    message_text = (
        f"Вы уверены, что хотите изменить время ухода для сотрудника <b>{user_name}</b> "
        f"на <b>{new_checkout_dt.strftime('%d.%m.%Y %H:%M')}</b>?"
    )
    keyboard = [
        [
            InlineKeyboardButton("✅ Да, изменить", callback_data="edit_confirm:yes"),
            InlineKeyboardButton("❌ Нет, назад", callback_data="edit_confirm:no")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def edit_confirm_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Финальное подтверждение. Обновляет запись в БД или отменяет."""
    query = update.callback_query
    await query.answer()
    ...
    if decision == 'yes':
        session_id = context.user_data['edit_target_session_id']
        new_checkout_dt = context.user_data['edit_new_checkout_dt']
        
        # Форматируем в строку для записи в БД
        new_checkout_db_str = new_checkout_dt.strftime('%Y-%m-%d %H:%M:%S')
        
        success = db.update_session_checkout_time(session_id, new_checkout_db_str)
        
        if success:
            await query.edit_message_text("✅ Время ухода успешно изменено!")
        else:
            await query.edit_message_text("❌ Произошла ошибка при обновлении данных в базе.")
        ...
        await query.message.reply_text("Действие отменено. Пожалуйста, введите новое время ухода или /cancel для выхода.")
        return EDIT_AWAIT_NEW_TIME
...
async def edit_receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    (ФИНАЛЬНАЯ ВЕРСИЯ)
    Принимает ФИО, ищет в базе и предлагает выбрать.
    Кнопки содержат ID для ясности и есть кнопка "Назад".
    """
    user_input_name = update.message.text
    found_users = db.find_users_by_name(user_input_name)
    ...
    for user in found_users:
        callback_data = f"edit_select_user:{user['telegram_id']}"
        # Улучшение №1: Добавляем ID пользователя в текст кнопки
        button_text = f"{user['application_full_name']} (ID: {user['telegram_id']})"
        keyboard.append([InlineKeyboardButton(button_text, callback_data=callback_data)])
    
    # Улучшение №2: Кнопка "Отмена" заменена на "Назад"
    keyboard.append([InlineKeyboardButton("↩️ Назад (ввести другое ФИО)", callback_data="edit_back_to_name")])
    # --- КОНЕЦ ИЗМЕНЕНИЙ ---
...
async def edit_cancel_dialog(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Универсальная функция отмены диалога.
    Работает и для команды /cancel, и для кнопок.
    """
    # Эта функция может быть вызвана через /cancel или через кнопку, если она есть
    ...
    # Очищаем все временные данные диалога
    context.user_data.clear()
    return ConversationHandler.END

async def fallback_expired_session(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Сообщает пользователю, что диалог, к которому относилась кнопка, истек.
    """
    query = update.callback_query
    try:
        await query.answer(
            text="⚠️ Этот диалог устарел. Пожалуйста, начните заново.",
            show_alert=True  # Показываем всплывающее уведомление
        )
    except Exception as e:
        logger.warning(f"Не удалось отправить уведомление об истекшем диалоге: {e}")

async def clear_conversation_state(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    "Перехватчик", который очищает состояние предыдущего диалога
    перед запуском нового.
    """
    if context.user_data:
        logger.info(f"User {update.effective_user.id} начинает новый диалог. Сброс старого состояния.")
        context.user_data.clear()
...
async def notify_admins_new_manual_request(bot: Bot, requesting_user: User, requested_time: datetime):
    """Уведомляет администраторов, ИСПОЛЬЗУЯ ФИО ИЗ БАЗЫ ДАННЫХ."""
    logger.info(f"Подготовка уведомления администраторам о заявке от user_id={requesting_user.id}")
    
    # Шаг 1: Получаем полные данные пользователя из нашей базы.
    user_profile_row = db.get_user(requesting_user.id)
    
    # --- НАЧАЛО ГЛАВНОГО ИСПРАВЛЕНИЯ ---
    # Шаг 2: Определяем, какое имя использовать.
    if user_profile_row and user_profile_row.get('application_full_name'):
        # Приоритет: официальное ФИО из нашей базы.
        display_name = user_profile_row['application_full_name']
    else:
        # Запасной вариант: имя из профиля Telegram.
        display_name = requesting_user.full_name
        logger.warning(f"Не найдено ФИО в базе для user_id={requesting_user.id}. Используется имя из Telegram.")
    
    full_name_escaped = escape_markdown_v2(display_name)
    # --- КОНЕЦ ГЛАВНОГО ИСПРАВЛЕНИЯ ---


    # Твоя правильная логика определения сектора (остается без изменений)
    ...
    # Используем новое `full_name_escaped` в сообщении
    message_text = (
        f"‼️ Новая заявка на ручную отметку прихода\\!\n\n"
        f"👤 **Сотрудник:** {full_name_escaped} \\(@{username_escaped}\\)\n"
...
async def admin_manual_checkins_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Начинает диалог: показывает список ожидающих заявок с ФИО из базы."""
    ...
    for req in pending_requests:
        # --- НАЧАЛО ГЛАВНОГО ИСПРАВЛЕНИЯ: ИСПОЛЬЗУЕМ ФИО ИЗ БАЗЫ ---
        # Приоритет: официальное ФИО. Запасной вариант: username.
        ...
        button_text = f"{display_name} на {time_str}"
        # --- КОНЕЦ ГЛАВНОГО ИСПРАВЛЕНИЯ ---
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f"admin_process_req_{req['request_id']}")])
...
async def admin_select_manual_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Показывает детали одной заявки. ТЕПЕРЬ УСТОЙЧИВА к вызову с разных экранов.
    """
    query = update.callback_query
    await query.answer()


    # --- НАЧАЛО ГЛАВНОГО ИСПРАВЛЕНИЯ: ГИБКОЕ ПОЛУЧЕНИЕ ID ---
    try:
        # Попытка №1: Получить ID из callback_data (стандартный путь из списка)
        request_id = int(query.data.split('_')[-1])
    except (ValueError, IndexError):
        # Попытка №2: Если не вышло (например, data='..._back'), берем ID из памяти.
        req_data_from_memory = context.user_data.get('current_manual_request')
        if not req_data_from_memory:
            await query.edit_message_text("Ошибка: данные о заявке утеряны. Начните заново с /admin_manual_checkins.", reply_markup=None)
            return ConversationHandler.END
        request_id = req_data_from_memory['request_id']
    # --- КОНЕЦ ГЛАВНОГО ИСПРАВЛЕНИЯ ---
    ...
    if not req or req['status'] != 'pending':
        ...
        # Очищаем память, чтобы не было проблем при следующем заходе
        context.user_data.pop('current_manual_request', None)
        return await admin_manual_checkins_start(update, context) # Возвращаемся к списку


    context.user_data['current_manual_request'] = req
    ...
    keyboard = [
        [InlineKeyboardButton("✅ Одобрить как есть", callback_data=f"admin_req_approve_as_is_{req['request_id']}")],
        [InlineKeyboardButton("🕒 Одобрить с другим временем", callback_data=f"admin_req_change_time_{req['request_id']}")],
        [InlineKeyboardButton("❌ Отклонить", callback_data=f"admin_req_reject_{req['request_id']}")],
...
async def admin_receive_new_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Обрабатывает новое время от админа и формирует подтверждение с ФИО из базы.
    """
    user_input_time_str = update.message.text
    try:
        # Валидация и обработка времени (эта часть кода у тебя правильная)
        ...
        # --- НАЧАЛО ГЛАВНОГО ИСПРАВЛЕНИЯ: ИСПОЛЬЗУЕМ ФИО ИЗ БАЗЫ ---
        # Приоритет: официальное ФИО из данных, которые мы уже получили от базы.
        # Запасной вариант: username.
        display_name = req.get('application_full_name') or f"@{req.get('username', 'N/A')}"
        # --- КОНЕЦ ГЛАВНОГО ИСПРАВЛЕНИЯ ---
        
        message = f"Одобрить заявку для <b>{display_name}</b> с новым временем <b>{aware_new_time.strftime('%d.%m.%Y %H:%M')}</b> (МСК)?"
        keyboard = [[InlineKeyboardButton("✅ Да", callback_data="admin_confirm_final_yes"), InlineKeyboardButton("❌ Нет", callback_data="admin_confirm_final_no_back")]]
...
async def restart_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Эта команда, будучи вызванной из `fallbacks` диалога,
    принудительно завершает его и очищает user_data.
    """
    user_id = update.effective_user.id
    context.user_data.clear()
    await update.message.reply_text("✅ Ваш диалог был успешно сброшен. Можете начинать заново.")
    logger.info(f"Пользователь {user_id} принудительно сбросил свой диалог с помощью /restart.")
    
    # Возвращаем константу, которая ГАРАНТИРОВАННО завершает любой диалог
    return ConversationHandler.END
...
async def set_bot_commands(application: Application):
    ...
    authorized_user_commands = [
        ...
        BotCommand("request_manual_checkin", "🛠️ Запросить ручной приход"), # <--- ВОТ ЗДЕСЬ ДОБАВЛЯЕМ
    ]
...
async def edit_back_to_sessions_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    # Просто пересобираем список с последним известным смещением (или с 0)
    offset = context.user_data.get('last_offset', 0)
    message_text, reply_markup = await edit_build_session_list(context, offset)
...
async def post_init_setup(application: Application) -> None:
    """
    Выполняет асинхронные задачи после инициализации приложения,
    например, установку команд бота.
    """
    logger.info("Установка команд бота...")
    # Убедись, что функция set_bot_commands у тебя определена где-то в файле
    await set_bot_commands(application)
    logger.info("Команды бота успешно установлены.")

async def handle_conversation_timeout(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Информирует пользователя, что диалог истек по таймауту.
    """
    # Удаляем данные, чтобы диалог точно завершился
    context.user_data.clear()
    
    if update.callback_query:
        query = update.callback_query
        await query.answer("Этот диалог истек по времени. Начните заново.", show_alert=True)
        # Можно также отредактировать сообщение, чтобы убрать кнопки
        await query.edit_message_text("Диалог завершен из-за неактивности.")
    elif update.message:
        await update.message.reply_text("Диалог завершен из-за неактивности. Начните заново.")
...
def main() -> None:
    ...
    # --- ОПРЕДЕЛЕНИЕ ВСЕХ CONVERSATION HANDLER'ов ---
    ...
    export_conv_handler = ConversationHandler(
        # ИСПРАВЛЕНИЕ №1: ДОБАВЛЕН ФИЛЬТР
        entry_points=[CommandHandler("admin_export_attendance", start_export_dialog, filters=AdminFilter())],
        states={
            SELECT_SECTOR: [CallbackQueryHandler(select_sector_callback, pattern="^export_sector_(СС|ВИ|ОП|ALL)$")],
            SELECT_PERIOD: [
        ...
        fallbacks=[CallbackQueryHandler(cancel_export_dialog, pattern="^export_cancel_dialog$")],
        name="export_flow",
        per_user=True,
        per_chat=True,
        allow_reentry=True
    ...
    admin_manual_checkins_conv_handler = ConversationHandler(
        # ИСПРАВЛЕНИЕ №2: ДОБАВЛЕН ФИЛЬТР
        entry_points=[CommandHandler("admin_manual_checkins", admin_manual_checkins_start, filters=AdminFilter())],
        states={
            ...
            ADMIN_PROCESS_SINGLE_REQUEST: [CallbackQueryHandler(admin_process_request_action, pattern="^admin_req_")],
            ADMIN_ENTER_NEW_TIME: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_receive_new_time)],
            ADMIN_CONFIRM_REQUEST_DECISION: [CallbackQueryHandler(admin_handle_final_confirmation, pattern="^admin_confirm_final_")],
        },
        fallbacks=[CallbackQueryHandler(admin_cancel_manual_checkins_dialog, pattern="^admin_cancel_manual_dialog$")],
    ...
    edit_checkout_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("edit_checkout", edit_checkout_start, filters=AdminFilter())],
        states={
            EDIT_AWAIT_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_receive_name)],
            EDIT_SELECT_USER: [
                CallbackQueryHandler(edit_select_user_callback, pattern="^edit_select_user:"),
                CallbackQueryHandler(edit_back_to_name_input, pattern="^edit_back_to_name$")
            ],
            EDIT_SELECT_PERIOD: [
                CallbackQueryHandler(edit_select_period_callback, pattern="^edit_period:"),
                CallbackQueryHandler(edit_back_to_name_input, pattern="^edit_back_to_name$")
            ],
            EDIT_SELECT_SESSION: [
                CallbackQueryHandler(edit_paginate_sessions_callback, pattern="^edit_paginate:"),
                CallbackQueryHandler(edit_back_to_period_selection, pattern="^edit_back_to_period$"),
                CallbackQueryHandler(edit_session_selected_callback, pattern="^edit_select_session:")
            ],
            EDIT_AWAIT_NEW_TIME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, edit_receive_new_time),
                CallbackQueryHandler(edit_back_to_sessions_list, pattern="^edit_back_to_sessions$")
            ],
            EDIT_CONFIRM: [
                CallbackQueryHandler(edit_confirm_update, pattern="^edit_confirm:")
            ],
            ConversationHandler.TIMEOUT: [handle_conversation_timeout]
        },
        fallbacks=[CommandHandler("cancel", edit_cancel_dialog)],
    ...
    application.add_handler(CommandHandler("admin_authorize", admin_authorize_command, filters=AdminFilter()))
    application.add_handler(CommandHandler("admin_pending_users", admin_pending_users_command, filters=AdminFilter()))
